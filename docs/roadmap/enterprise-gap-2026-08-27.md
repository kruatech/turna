...
| Credential rotation without downtime | P0 | **done** — certificates and shared secret | Certificate rotation verified under load: counter 0 -> 1, no reload failures, 36 021 frames relayed with zero errors across the swap. The shared secret is now hot too: SIGHUP re-reads the config file and republishes the SharedSecret backends, so a rotation moves through its two-secret window without a restart between the steps. `UpdateConfig` still does not carry the secret, deliberately — a credential should not travel over the management API (`docs/OPEN-DECISIONS.md` §0). | `scripts/verify/rotation-under-load.sh` checks both halves — live sessions survive, and the old secret stops granting new allocations. Its secret phase now checks the node is alive and ready after SIGHUP, and requires `event = "secret_reloaded"` in the log, before any credential assertion counts: the earlier version SIGHUP'd a node with no handler and every remaining check passed against the corpse. |
