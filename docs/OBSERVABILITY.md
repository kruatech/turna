...
| `turna_rate_limiter_evictions_total` | counter | Rate-limiter entries dropped to make room for a new source. Some churn is normal — the table is finite and the client population changes. A sustained rate against a flat client count means a spoofed-source flood, or a real population larger than `max_entries`. New sources are admitted either way; that is the design. Before 0.5.0 a full table refused them instead. |
| `turna_unauth_replies_suppressed_total` | counter | Binding responses and 401 challenges not sent because the source used up its reflection budget (64 burst, 8/s). A legitimate client needs single digits of these in its whole life, so any sustained rate means one of two things: spoofed requests naming a victim as the source, or a client stuck in a challenge loop. The suppressed reply is dropped in silence — answering "you are rate limited" would be another unauthenticated reply to the same address. |
| `turna_recv_workers_alive` | gauge | Receive workers still running. Compare against the worker count the node was started with (`TURNA_WORKERS`, 0 = one per core, logged at startup). A fall is permanent — a worker that exits does not come back — and it means the clients the kernel hashes to that socket are unserved while the node still reports Ready. **Alert on any decrease.** |
| `turna_dtls_cookie_challenges_total` | counter | ClientHellos answered with a HelloVerifyRequest because the source address had not been validated. Every real client pays exactly one of these per handshake, so this should track new DTLS sessions one-for-one. Far above that, against a flat `accepted`, is a spoofed flood — and one that now costs an HMAC and a datagram instead of a connection and a task. |
| `turna_dtls_pending_handshakes` | gauge | Handshakes in flight: state allocated, outcome unknown. **Demux path only.** This is the number a spoofed-source flood drives up, because a ClientHello allocates a connection and a task before the cookie exchange has proved the source address is real. A value sitting near `max_pending_handshakes` with a flat `turna_dtls_accepted_total` is an attack, not load. |
| `turna_dtls_rejected_pending_cap_total` | counter | Datagrams dropped because `max_pending_handshakes` was reached. Any sustained rate means the listener is shedding handshake attempts — legitimate ones too, if the cap is genuinely too low for the client population. Read it together with the gauge above. |
...
| `turna_quic_retries_sent_total` | counter | Initials answered with a Retry because the source address was not yet validated (RFC 9000 §8.1). Under normal traffic this tracks new connections roughly one-for-one — every client pays one Retry on its first connection. A large value against few accepted connections means spoofed Initials, and each one cost a datagram instead of a TLS signature. |
