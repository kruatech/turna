...
/// Default sweep cadence — every 10s. With the default 5s heartbeat
/// interval, that means a dead node is noticed within at most
/// `live_window + sweep_interval` ≈ 30s + 10s = 40s. Tunable later.
pub const DEFAULT_SWEEP_INTERVAL: Duration = Duration::from_secs(10);
/// Default "live" window — 30s. Six missed heartbeats before failover.
    ...
    loop {
        tokio::select! {
            ...
            _ = ticker.tick() => {
                ...
                if let Err(e) = sweep_once(&backend, &store, &cfg, &mut stats).await {
...
pub(crate) async fn sweep_once(
    ...
    let dead: Vec<&str> = all.iter()
        .map(|n| n.node_id.as_str())
        .filter(|id| *id != me && !live_ids.contains(id))
        .collect();

    if dead.is_empty() {
        debug!("failover: no dead nodes to claim from");
        return Ok(());
    }
    stats.dead_nodes_seen += dead.len() as u64;
    info!(dead_count = dead.len(), "failover: dead nodes detected");
    ...
    for dead_id in dead {
        ...
        for stored in orphans {
            ...
            if !claimed {
                ...
                debug!(port, dead_id, "failover: claim lost (already taken or vanished)");
                continue;
            }
            ...
            let fresh = match backend.get_allocation(port).await {
                Ok(Some(a)) => a,
                Ok(None) => {
                    warn!(port, "failover: claimed allocation vanished before re-fetch");
            ...
            match bulk_load::apply_one(store, &fresh) {
                ...
                Err(reason) => {
                    ...
                    warn!(port, dead_id, reason,
                          "failover: rehydrate refused; reverting claim");
                    if let Err(e) = backend.claim_allocation(
                        port, &cfg.node_id, dead_id
                    ).await {
...
mod tests {
    ...
    fn now_ms() -> u64 { turna_session::epoch_ms() }
    ...
    fn live_hb(node_id: &str) -> NodeHeartbeat {
        NodeHeartbeat {
            ...
            uptime_secs: 60, version: "test".into(),
            last_seen_ms: now_ms(),
            draining: false,
        }
    }
    ...
    fn alloc_for(node: &str, port: u16) -> StoredAllocation {
        ...
        StoredAllocation {
            ...
            channels: vec![StoredChannel {
                number: 0x4000, peer_addr: "10.0.0.2:9000".into(),
                expires_at_ms: now_ms() + 600_000,
            }],
        }
    }
    ...
    async fn basic_claim_of_three_orphans() {
        ...
        for port in [40010, 40011, 40012] {
            backend.store_allocation(&alloc_for("node-dead", port)).await.unwrap();
        }

        let mut stats = FailoverStats::default();
        sweep_once(&backend, &store, &cfg("node-me"), &mut stats).await.unwrap();
    ...
    async fn does_not_claim_self() {
        let (backend, store) = fresh().await;
        backend.heartbeat(&live_hb("node-me")).await.unwrap();
        backend.store_allocation(&alloc_for("node-me", 40020)).await.unwrap();
    ...
    async fn does_not_claim_live_peers() {
        ...
        backend.store_allocation(&alloc_for("node-peer", 40030)).await.unwrap();
        ...
        assert_eq!(a.node_id, "node-peer", "peer's allocation must not be touched");
    }
    ...
    async fn cas_serialises_concurrent_claims() {
        ...
        backend.store_allocation(&alloc_for("node-dead", 40040)).await.unwrap();

        let b1 = backend.clone();
        let b2 = backend.clone();
        let t1 = tokio::spawn(async move {
            b1.claim_allocation(40040, "node-dead", "node-A").await
        });
        let t2 = tokio::spawn(async move {
            b2.claim_allocation(40040, "node-dead", "node-B").await
        });
        let r1 = t1.await.unwrap().unwrap();
        let r2 = t2.await.unwrap().unwrap();
        assert!(r1 ^ r2, "exactly one of the two CAS attempts must succeed");
        let owner = backend.get_allocation(40040).await.unwrap().unwrap().node_id;
    ...
    async fn rehydrate_failure_reverts_claim() {
        ...
        backend.heartbeat(&live_hb("node-me")).await.unwrap();
        backend.heartbeat(&stale_hb("node-dead")).await.unwrap();
        backend.store_allocation(&alloc_for("node-dead", 40050)).await.unwrap();
        ...
        assert!(stats.claim_revert_attempted >= 1,
        ...
        let owner = backend.get_allocation(40050).await.unwrap().unwrap().node_id;
        assert_eq!(owner, "node-dead",
