...
if [ "${SKIP_BUILD:-0}" != 1 ]; then
  command -v cargo >/dev/null || die "cargo is not on PATH; build as your user then use SKIP_BUILD=1"
  command -v clang >/dev/null || die "clang is required for BPF compilation"
fi
...
# Never delete or reuse another run's namespace/link.
ip link show "$DEV0" >/dev/null 2>&1 && die "interface $DEV0 already exists"
ip netns list | awk '{print $1}' | grep -Fxq "$NS" && die "namespace $NS already exists"
umask 077
mkdir "$OUT" || die "output directory already exists"
python3 - <<'CHECK_PORT'
import socket
with socket.socket() as s: s.bind(('127.0.0.1', 19100))
CHECK_PORT
[ "$?" -eq 0 ] || die "health port 19100 is occupied"
...
stop_node() {
  ...
  for _ in $(seq 120); do kill -0 "$NODE_PID" 2>/dev/null || break; sleep 0.5; done
  if kill -0 "$NODE_PID" 2>/dev/null; then
    say "FAIL node shutdown exceeded 60s"
    kill -KILL "$NODE_PID" 2>/dev/null || true
    rc=1
  fi
  wait "$NODE_PID" 2>/dev/null || rc=1
  NODE_PID=""
  say "node shutdown: exit=$rc"
  return "$rc"
}
cleanup() {
  local rc=$?
  trap - EXIT INT TERM
  stop_node || rc=1
  if [ "$LAB_LINK_CREATED" = 1 ]; then ip link del "$DEV0" 2>/dev/null || true; fi
  if [ "$LAB_NS_CREATED" = 1 ]; then ip netns del "$NS" 2>/dev/null || true; fi
  say "lab torn down"
  exit "$rc"
}
trap cleanup EXIT
...
cargo build --locked --release -p turna-node --features af-xdp > "$OUT/build-node.log" 2>&1 \
  || { tail -20 "$OUT/build-node.log"; die "node build failed — see $OUT/build-node.log"; }
cargo build --locked --release -p turna-load-test > "$OUT/build-load.log" 2>&1 \
  || { tail -20 "$OUT/build-load.log"; die "load-test build failed"; }

fi
[ -x target/release/turna-node ] && [ -x target/release/turna-load-test ] || die "build node and load client first"
...
setup_lab() {
  ...
  LAB_NS_CREATED=1
  ip link add "$DEV0" type veth peer name "$DEV1" || return 1
  LAB_LINK_CREATED=1
  ip link set "$DEV1" netns "$NS" || return 1
  ethtool -K "$DEV0" tx off rx off tso off gso off gro off || return 1
  ip netns exec "$NS" ethtool -K "$DEV1" tx off rx off tso off gso off gro off || return 1
  ip addr add "$IP0/24" dev "$DEV0" || return 1
  ip link set "$DEV0" up || return 1
  ip netns exec "$NS" ip addr add "$IP1/24" dev "$DEV1" || return 1
  ip netns exec "$NS" ip link set "$DEV1" up || return 1
  ip netns exec "$NS" ip link set lo up || return 1
}
setup_lab > "$OUT/veth.log" 2>&1 || { cat "$OUT/veth.log"; die "veth/netns setup failed"; }
...
# Pin neighbors only to exclude cold-start resolution from conformance timing.
# The selective filter passes ARP/NDP to the kernel; no redirect-all program is used.
...
READY=0
for i in $(seq 40); do
  if curl -fsS --max-time 1 http://127.0.0.1:19100/ready >/dev/null 2>&1; then READY=1; break; fi
...
AFXDP_READY=0
for _ in $(seq 40); do
  v="$(curl -fsS --max-time 1 http://127.0.0.1:19100/metrics 2>/dev/null | awk '$1=="turna_afxdp_readiness"{print $2}')"
...
if sent <= 0 or recv < 0 or recv > sent:
...
# uses strict zero-loss acceptance with the corrected draining load client.
run "AF_XDP relayed media, low rate" media-low.log \
  ip netns exec "$NS" "$LOAD" --server "$IP0:3478" --secret afxdp-lab-secret \
  --bind-ip "$IP1" --duration 15 --json channel-data --channels 4 --pps 10 --payload 160
assert_loss "low-rate media" "$OUT/media-low.log" 0 || FAIL=$((FAIL + 1))
...
assert_loss "low-rate media past the pool" "$OUT/media-long.log" 0 || FAIL=$((FAIL + 1))
say "  (frames received so far: $(curl -fsS http://127.0.0.1:19100/metrics 2>/dev/null | awk '$1=="turna_afxdp_rx_frames_total"{print $2}'))"
...
assert_loss "high-rate media" "$OUT/media-high.log" 0 || FAIL=$((FAIL + 1))

# Same for the datapath itself: if AF_XDP received no frames, whatever was measured
# did not go through it.
RX="$(curl -fsS http://127.0.0.1:19100/metrics 2>/dev/null | awk '$1=="turna_afxdp_rx_frames_total"{print $2}')"
TX="$(curl -fsS http://127.0.0.1:19100/metrics 2>/dev/null | awk '$1=="turna_afxdp_tx_frames_total"{print $2}')"
...
curl -fsS http://127.0.0.1:19100/metrics 2>/dev/null \
