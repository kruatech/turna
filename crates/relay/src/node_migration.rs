...
//!
//! # UNWIRED — читай это прежде, чем поверить в написанное выше
//!
//! Ни одна из структур этого модуля никуда не вызывается. Единственная ссылка
//! извне — строка `pub mod node_migration;` в `crates/relay/src/lib.rs`; проверено
//! грепом по `crates`, `services`, `tools` и `tests`. Внутренние тесты проходят,
//! но они тестируют координацию, которую никто не запускает.
//!
//! Практический вывод: **cross-node migration не работает** — нет пути, который
//! реально переносит аллокацию на другую ноду. То, что работает, живёт в
//! `turna_transport::migration` (тикеты MOBILITY-TICKET, ReKey, migration epoch) и
//! подключено в `processor.rs` / `server.rs` / `main.rs`. Это same-node mobility по
//! RFC 8016, и не более.
//!
//! Раньше документация называла это «unverified», что вводило в заблуждение:
//! непроверенное и неподключённое — разные вещи. Сейчас `README.md`,
//! `docs/feature-support.md`, `docs/PRODUCTION_READINESS.md` и
//! `docs/protocol-gap.md` говорят «unwired», а `scripts/check-doc-claims.sh`
//! падает, если модуль остался без вызовов, а формулировка из доков исчезла.
//!
//! # Два выхода из этого состояния
//!
//! 1. **Подключить.** Нужен перенос [`MigrationPayload`] на целевую ноду по
//!    control-plane gRPC, fencing (чтобы обе ноды не обслуживали одну аллокацию
//!    одновременно), и маршрутизация relay-порта на новом хосте. Это отдельная
//!    работа, сопоставимая с самим модулем, и без неё код здесь бесполезен.
//! 2. **Удалить.** Модуль с тестами читается как существующая возможность —
//!    именно так и получилось расхождение в доках. Если cross-node миграция не в
//!    планах релиза, удаление честнее сохранения.
//!
//! Решение не принято. Пока оно не принято, этот комментарий — то, что удерживает
//! читателя модуля от неверного вывода.
