...
enum OutMsg {
    Relay         { port: u16, data: Bytes, target: SocketAddr },
    RegisterRelay { port: u16, socket: std::net::UdpSocket },
    CloseRelay    { port: u16 },
}
...
pub fn recv_workers() -> usize {
    let default = if cfg!(target_os = "linux") {
        std::thread::available_parallelism().map(|n| n.get() * 2).unwrap_or(8).min(32)
    } else {
        1
    };
...
impl RelayServer {
    pub fn new(
        ...
        let processor = Arc::new(PacketProcessor::new(store, auth, external_ip, metrics));
    ...
    pub fn processor(&self) -> &Arc<PacketProcessor> { &self.processor }
    ...
    pub async fn run(
        ...
        let relay_tasks: Arc<DashMap<u16, tokio::task::AbortHandle>> =
            Arc::new(DashMap::new());
                ...
                loop {
                    ...
                    if removed > 0 {
                        metrics.active_allocations.fetch_sub(
                            removed as u64, std::sync::atomic::Ordering::Relaxed,
                        );
                    ...
                    let orphans: Vec<u16> = relay_sockets_cl.iter()
                        .map(|e| *e.key())
                    ...
                    metrics.rtp_streams.store(agg.total_streams, std::sync::atomic::Ordering::Relaxed);
                    metrics.rtp_avg_loss_pct_x100.store((agg.avg_loss_percent * 100.0) as u64, std::sync::atomic::Ordering::Relaxed);
                    metrics.rtp_max_loss_pct_x100.store((agg.max_loss_percent * 100.0) as u64, std::sync::atomic::Ordering::Relaxed);
                    metrics.rtp_avg_jitter_us.store((agg.avg_jitter_ms * 1000.0) as u64, std::sync::atomic::Ordering::Relaxed);
                    metrics.rtp_max_jitter_us.store((agg.max_jitter_ms * 1000.0) as u64, std::sync::atomic::Ordering::Relaxed);
                    metrics.rtp_total_bitrate_kbps.store(agg.total_bitrate_bps / 1000, std::sync::atomic::Ordering::Relaxed);
                    rtp.cleanup_stale();
                }
            });
        }
                ...
                match msg {
                    ...
                    OutMsg::RegisterRelay { port, socket } => {
                        ...
                        let socket = match tokio::net::UdpSocket::from_std(socket) {
                            ...
                            Err(e) => { error!(port, %e, "failed to adopt relay socket"); continue; }
                        };
        ...
        for i in 0..n_workers {
            ...
            workers.push(tokio::spawn(async move {
                ...
                loop {
                    ...
                    unsafe { arena.set_len(BATCH * MAX_UDP_PACKET); }

                    let n = {
                        let mut slots: Vec<&mut [u8]> =
                            arena.chunks_mut(MAX_UDP_PACKET).collect();
                        match transport.recv_mmsg(&mut slots, &mut metas).await {
                            Ok(n)  => n,
                            Err(e) => { error!(worker = i, %e, "recv error, worker stopping"); break; }
                        }
                    };
                        ...
                        for action in processor.process(raw, src) {
                            match action {
                                ...
                                Action::SendViaRelay { data, target, relay_port } => {
                                    if send_tx.try_send(OutMsg::Relay { port: relay_port, data, target }).is_err() {
                                        processor.metrics().send_queue_dropped
                                            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                                    }
                                }
                                // Control-plane: не дропаем при полной очереди.
                                Action::RegisterRelay { port, socket, .. } => {
                                    if send_tx.send(OutMsg::RegisterRelay { port, socket }).await.is_err() {
                                        break;
                                    }
                                }
    ...
    async fn drain(&self) {
        ...
        while store.len() > 0 && tokio::time::Instant::now() < timeout {
            let removed = store.cleanup_expired();
            if removed > 0 {
                self.processor.metrics().active_allocations.fetch_sub(
                    removed as u64, std::sync::atomic::Ordering::Relaxed,
                );
            }
