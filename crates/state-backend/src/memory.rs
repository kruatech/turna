...
impl InMemoryBackend {
    ...
    pub async fn ping(&self) -> Result<()> { Ok(()) }
}
...
mod tests {
    ...
    async fn heartbeat_and_nodes() {
        ...
        b.heartbeat(&NodeHeartbeat {
            node_id: "n1".into(), addr: "10.0.0.1:3478".into(),
            active_allocations: 5, total_bandwidth_bps: 1000,
            cpu_usage_pct: 10.0, memory_usage_pct: 20.0,
            uptime_secs: 60, version: "0.1.0".into(),
            last_seen_ms: now_ms(), draining: false,
        }).await.unwrap();
    ...
    async fn claim_allocation_fails_on_mismatch() {
        ...
        let ok = b.claim_allocation(50001, "node-XYZ", "node-2").await.unwrap();
        assert!(!ok);
        let a = b.get_allocation(50001).await.unwrap().unwrap();
        assert_eq!(a.node_id, "node-1",
                   "owner must not change on CAS mismatch");
    }
    ...
    async fn get_live_nodes_saturates_on_huge_max_age() {
        let b = InMemoryBackend::new();
        b.heartbeat(&NodeHeartbeat {
            node_id: "n1".into(), addr: "1.2.3.4:1".into(),
            active_allocations: 0, total_bandwidth_bps: 0,
            cpu_usage_pct: 0.0, memory_usage_pct: 0.0,
            uptime_secs: 0, version: "v".into(),
            last_seen_ms: now_ms(), draining: false,
        }).await.unwrap();
