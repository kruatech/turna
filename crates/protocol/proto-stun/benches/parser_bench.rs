...
fn classify(data: &[u8]) -> u8 {
    ...
    if f >= 0x4000 && f <= 0x7FFF { return 1; }
    if data.len() >= 20 && data[0] & 0xC0 == 0 && u32::from_be_bytes([data[4],data[5],data[6],data[7]]) == MAGIC { return 2; }
    0
}

fn bench_classify(c: &mut Criterion) {
    let s = stun_pkt(0); let ch = channel_pkt(160);
