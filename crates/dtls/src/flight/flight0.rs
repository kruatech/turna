//! **Modified from webrtc-dtls 0.10.0.** Honours
//! `Config::insecure_skip_verify_hello`: when the caller has already proved the
//! client's address, this flight accepts the ClientHello it was given — at
//! whatever `message_seq` it carries — and goes straight to Flight4 instead of
//! answering with a HelloVerifyRequest of its own.
...
impl Flight for Flight0 {
    async fn parse(
        ...
        // Which sequence number to demand of the ClientHello.
        //
        // `full_pull_map` requires an exact match, not a lower bound — it
        // reports "There is a gap" and gives up otherwise. Zero is right for a
        // client that has not been challenged. A client that answered an
        // external HelloVerifyRequest sends `message_seq = 1`, and demanding
        // zero there makes the state machine wait for a message that will never
        // arrive: the handshake stalls in silence until the accept timeout.
        // That was the observed failure before this flag existed.
        let start_seq = if cfg.insecure_skip_verify_hello {
            match cache.current_client_hello_sequence(cfg.initial_epoch).await {
                Some(n) => n,
                // Nothing to read yet; the caller retries when more arrives.
        ...
        if let Some(message) = msgs.get(&HandshakeType::ClientHello) {
            ...
            if cfg.insecure_skip_verify_hello {
                // The address was proved before this connection existed, so the
                // cookie exchange has already happened — one layer up. Running
                // it again cannot work: this flight would issue a cookie of its
                // own and the client would answer the one it already has.
                Ok(Box::new(Flight4 {}))
            } else {
                Ok(Box::new(Flight2 {}))
            }
    ...
    async fn generate(
        ...
        // lost its meaning is how the next reader concludes the exchange still
        // happens here.
