//! **Modified from webrtc-dtls 0.10.0.** Threads
//! `Config::insecure_skip_verify_hello` through to the handshake config.
    ...
    pub async fn new(
        ...
        */
        ...
        let cfg = HandshakeConfig {
            ...
            insecure_skip_verify_hello: config.insecure_skip_verify_hello,
        ...
        let (state, flight, initial_fsm_state) = if let Some(state) = initial_state {
            ...
            // Handshake sequence numbers start at 1 for a server whose caller
            // validated the client's address itself.
            //
            // An external HelloVerifyRequest already consumed sequence 0
            // (RFC 6347 §4.2.4), so the client's second ClientHello arrives at
            // message_seq 1 and our ServerHello must leave at message_seq 1 —
            // Certificate 2, ServerHelloDone 3.
            //
            // Both counters matter, for different reasons. `recv` is what the
            // flight parsers ask the handshake cache for; at 0 they look for a
            // message that does not exist. `send` numbers our outgoing
            // messages; at 0 the ServerHello goes out as sequence 0, and the
            // client rejects the handshake at Finished — the message sequence
            // is part of the transcript hash (RFC 6347 §4.2.6), so the two
            // sides compute different MACs and the failure surfaces far from
            // its cause.
            let resumed_after_external_cookie = !is_client && cfg.insecure_skip_verify_hello;
            let start_seq = isize::from(resumed_after_external_cookie);

            (
                State {
                    is_client,
                    handshake_recv_sequence: start_seq,
                    handshake_send_sequence: start_seq,
                    ..Default::default()
                },
                flight,
        ...
        // Read before `cfg` moves into the connection below: the reader task
        // needs this value, and by then the config belongs to someone else.
        let fragment_start_seq = u16::from(!is_client && cfg.insecure_skip_verify_hello);
        ...
        tokio::spawn(async move {
            ...
            let mut ctx = ConnReaderContext {
                ...
                // Starts at 1 when the caller validated the address itself.
                //
                // That path means an external HelloVerifyRequest already went
                // out and consumed sequence 0 (RFC 6347 §4.2.4), so the client's
                // ClientHello carries message_seq = 1. A buffer starting at 0
                // files it under key 1 and never looks there.
                fragment_buffer: FragmentBuffer::starting_at(fragment_start_seq),
                cache: cache2,
