...
impl FragmentBuffer {
    ...
    /// A buffer that expects the next handshake message to carry
    /// `message_seq == start`.
    ///
    /// **Added in turna's copy; not in webrtc-dtls 0.10.0.**
    ///
    /// `pop` only yields the message filed under
    /// `current_message_sequence_number`, which is correct for a handshake this
    /// side drives from the beginning and wrong for one that resumes. A client
    /// answering an externally issued HelloVerifyRequest sends its second
    /// ClientHello at `message_seq = 1` — RFC 6347 §4.2.4, the
    /// HelloVerifyRequest consumed sequence 0 — and a buffer starting at 0 files
    /// that message under key 1 and never looks there.
    ///
    /// The failure mode is what makes this worth a paragraph: the message is
    /// not dropped, not rejected and not logged. It is held. The handshake dies
    /// at the accept timeout with nothing anywhere to say why, which is how it
    /// cost an evening to find.
    pub fn starting_at(start: u16) -> Self {
        FragmentBuffer {
            cache: HashMap::new(),
            current_message_sequence_number: start,
        }
    }
