//! **Modified from webrtc-dtls 0.10.0.** Adds
//! [`HandshakeCache::current_client_hello_sequence`], so a flight can pull a
//! ClientHello whose `message_seq` is not zero.
...
impl HandshakeCache {
    ...
    /// The `message_seq` of the highest ClientHello the client has sent, if any.
    ///
    /// **Added in turna's copy; not in webrtc-dtls 0.10.0.**
    ///
    /// `full_pull_map` matches the sequence number exactly, so a caller has to
    /// know it in advance. That is fine for a handshake that starts at zero and
    /// counts up, and wrong for one whose first message was already answered
    /// elsewhere: a client that responded to an external HelloVerifyRequest
    /// sends `message_seq = 1`, and asking for zero stalls the state machine in
    /// silence.
    ///
    /// The highest rather than the lowest, to match `full_pull_map`'s own
    /// selection: on a retransmit both ClientHellos are in the cache, and the
    /// later one is the one that carries the cookie.
    pub(crate) async fn current_client_hello_sequence(&self, epoch: u16) -> Option<isize> {
        let cache = self.cache.lock().await;
        cache
            .iter()
            .filter(|c| {
