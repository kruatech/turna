//! **Modified from webrtc-dtls 0.10.0.** Added
//! [`Config::insecure_skip_verify_hello`] so a listener that has already proved
//! the client's address can skip the in-connection cookie exchange. See the
//! field's documentation and the crate root for why.
...
pub struct Config {
    ...
    /// Accept the first ClientHello without running the HelloVerifyRequest
    /// cookie exchange.
    ///
    /// **On its own this removes the protection RFC 6347 §4.2.1 exists to
    /// provide.** Without the exchange a spoofed ClientHello allocates a
    /// connection, and the server becomes both an amplifier and an easy
    /// out-of-memory target. Do not set it because a round trip is annoying.
    ///
    /// It is here for exactly one caller: a listener that has already proved
    /// the client's address by some other means. turna's DTLS demultiplexer
    /// does that statelessly before allocating anything — HMAC over the address
    /// and a time bucket — and only then constructs a connection with this flag
    /// set. The address is verified; it is verified one layer up.
    ///
    /// Two mechanisms cannot both run: `flight0` generates its own random
    /// cookie and `flight2` compares against that, so a cookie issued anywhere
    /// else can never match.
    ///
    /// Named after pion/dtls's `InsecureSkipVerifyHello`, which is the same
    /// switch for the same reason.
    pub insecure_skip_verify_hello: bool,
